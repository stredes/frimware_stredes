...
static bool isValidPin(gpio_num_t pin) { return pin != GPIO_NUM_NC && (int)pin >= 0; }

static bool isSpiPinoutConfigured(const BruceConfigPins::SPIPins &bus, bool needsIo0 = true) {
    bool basePins = isValidPin(bus.sck) && isValidPin(bus.miso) && isValidPin(bus.mosi) && isValidPin(bus.cs);
    return needsIo0 ? basePins && isValidPin(bus.io0) : basePins;
}

static bool isDigitalPinoutConfigured(int tx, int rx) {
    return tx >= 0 && rx >= 0;
}

static String pinoutState(bool configured) {
    return configured ? "PIN OK" : "PIN OFF";
}

static void drawModuleDipStatus(const char *title) {
    if (title == nullptr) return;

    String badge = "";
    uint16_t color = bruceConfig.secColor;
    String section = String(title);

    if (section == "NRF24") {
        bool pinOk = isSpiPinoutConfigured(bruceConfigPins.NRF24_bus);
        badge = "DIP 2 NRF24 " + pinoutState(pinOk);
        color = pinOk ? TFT_GREEN : TFT_RED;
    } else if (section.startsWith("Radio Frequency")) {
        bool ccPinOk = isSpiPinoutConfigured(bruceConfigPins.CC1101_bus);
        bool ccSelected = bruceConfigPins.rfModule == CC1101_SPI_MODULE;
        bool askPinOk = isDigitalPinoutConfigured(bruceConfigPins.rfTx, bruceConfigPins.rfRx);
        bool askSelected = !ccSelected;

        if (ccSelected || ccPinOk) {
            badge = "DIP 1 CC1101 " + pinoutState(ccPinOk) + String(ccSelected ? " USO ON" : " USO OFF");
            color = ccPinOk ? (ccSelected ? TFT_GREEN : TFT_YELLOW) : TFT_RED;
        } else if (askSelected || askPinOk) {
            badge = "DIP 5 ASK/OOK " + pinoutState(askPinOk) + String(askSelected ? " USO ON" : " USO OFF");
            color = askPinOk ? (askSelected ? TFT_GREEN : TFT_YELLOW) : TFT_RED;
        } else {
            badge = "DIP 1/5 RF OFF";
            color = TFT_RED;
        }
    } else if (section.startsWith("RFID")) {
        bool on = bruceConfigPins.rfidModule == PN532_I2C_MODULE || bruceConfigPins.rfidModule == PN532_SPI_MODULE ||
                  bruceConfigPins.rfidModule == PN532_I2C_SPI_MODULE;
        badge = "DIP 3 PN532 " + String(on ? "USO ON" : "USO OFF");
        color = on ? TFT_GREEN : TFT_RED;
    } else if (section.startsWith("Infrared")) {
        bool pinOk = isDigitalPinoutConfigured(bruceConfigPins.irTx, bruceConfigPins.irRx);
        badge = "DIP 4 IR " + pinoutState(pinOk);
        color = pinOk ? TFT_GREEN : TFT_RED;
    }
    ...
    tft.setTextSize(FP);
    tft.fillRect(10, 40, tftWidth - 20, 10, bruceConfig.bgColor);
    tft.setTextColor(color, bruceConfig.bgColor);
    tft.drawString(badge, 12, 40, 1);
...
void TouchFooter(uint16_t color) {
    uint16_t accent = bruceConfig.secColor;
    tft.drawRoundRect(5, tftHeight + 2, tftWidth - 10, 43, 5, color);
    tft.drawRoundRect(8, tftHeight + 5, tftWidth - 16, 37, 4, accent);
    tft.drawFastVLine(tftWidth / 3, tftHeight + 8, 31, accent);
    tft.drawFastVLine(2 * tftWidth / 3, tftHeight + 8, 31, accent);
...
**********************************************************************/
static void applyTouchMenuNavigation(uint8_t menuType, int &index, const std::vector<Option> &options) {
#if defined(HAS_TOUCH)
    static bool pendingMainTouch = false;
    static int mainTouchStartX = 0;
    static int mainTouchStartY = 0;
    static unsigned long mainTouchStartMs = 0;
    static unsigned long lastMainTouchEventMs = 0;
    static const int MAIN_SWIPE_THRESHOLD = 40;
    static const int MAIN_TAP_MOVE_TOLERANCE = 18;
    static const unsigned long MAIN_TAP_TIMEOUT_MS = 220;

    if (menuType == MENU_TYPE_MAIN && pendingMainTouch &&
        millis() - lastMainTouchEventMs > MAIN_TAP_TIMEOUT_MS) {
        int dx = abs(mainTouchStartX - tftWidth / 2);
        int dy = abs(mainTouchStartY - tftHeight / 2);
        if (dx < (tftWidth / 3) && dy < (tftHeight / 3)) SelPress = true;
        pendingMainTouch = false;
    }

    if (!touchPoint.pressed || options.empty()) return;

    int x = touchPoint.x;
    int y = touchPoint.y;
    int footerTop = tftHeight;
    int thirdX = tftWidth / 3;
    ...
    if (menuType == MENU_TYPE_MAIN) {
        if (y >= footerTop) {
            if (x < thirdX) PrevPress = true;
            else if (x < thirdX * 2) SelPress = true;
            else NextPress = true;
            pendingMainTouch = false;
            return;
        }
        ...
        if (!pendingMainTouch) {
            pendingMainTouch = true;
            mainTouchStartX = x;
            mainTouchStartY = y;
            mainTouchStartMs = millis();
            lastMainTouchEventMs = mainTouchStartMs;
            return;
        }

        int dx = x - mainTouchStartX;
        int dy = y - mainTouchStartY;
        lastMainTouchEventMs = millis();

        if (abs(dx) >= MAIN_SWIPE_THRESHOLD && abs(dx) > abs(dy)) {
            if (dx < 0) NextPress = true;
            else PrevPress = true;
            pendingMainTouch = false;
            return;
        }

        if (abs(dx) > MAIN_TAP_MOVE_TOLERANCE || abs(dy) > MAIN_TAP_MOVE_TOLERANCE) {
            mainTouchStartX = x;
            mainTouchStartY = y;
            mainTouchStartMs = millis();
        }
    ...
    if (menuType != MENU_TYPE_MAIN && x <= 42 && y <= 34) {
        EscPress = true;
        return;
    }
    ...
    if (menuType == MENU_TYPE_REGULAR) {
        ...
        if (visibleCount > MAX_MENU_SIZE) visibleCount = MAX_MENU_SIZE;

        int rowHeight = FM * 8 + 4;
        int boxX = tftWidth * 0.10;
        int boxY = tftHeight / 2 - visibleCount * rowHeight / 2 - 5;
        int boxW = tftWidth * 0.8;
        int boxH = rowHeight * visibleCount + 10;
        int firstVisible = (index >= MAX_MENU_SIZE) ? index - MAX_MENU_SIZE + 1 : 0;

        if (x >= boxX && x <= boxX + boxW && y >= boxY && y <= boxY + boxH) {
            int tappedRow = (y - boxY - 5) / rowHeight;
            if (tappedRow < 0) tappedRow = 0;
            int tappedIndex = firstVisible + tappedRow;
            if (tappedIndex >= (int)options.size()) tappedIndex = options.size() - 1;
            index = tappedIndex;
            SelPress = true;
            return;
        }
        ...
        if (x < thirdX) PrevPress = true;
        else if (x > thirdX * 2) NextPress = true;
        else if (y < tftHeight / 2) UpPress = true;
        else DownPress = true;
        return;
    }
...
int loopOptions(
    ...
    while (1) {
        ...
        applyTouchMenuNavigation(menuType, index, options);
...
Opt_Coord drawOptions(
    ...
    for (i = 0; i < menuSize; i++) {
        if (i >= init) {
            int rowY = tft.getCursorY() + 2;
            int rowX = tftWidth * 0.10 + 4;
            int rowW = tftWidth * 0.8 - 8;
            int rowH = FM * 8 + 2;
            tft.fillRect(rowX, rowY, rowW, rowH, bgcolor);

            String text = "";
            if (i == index) {
                tft.drawFastVLine(rowX, rowY, rowH, selcolor);
                tft.drawFastHLine(rowX + 4, rowY + rowH - 1, rowW - 8, selcolor);
                tft.setTextColor(selcolor, bgcolor);
                text += ">";
                coord.x = tftWidth * 0.10 + 5 + FM * LW;
                coord.y = tft.getCursorY() + 4;
                coord.size = (tftWidth * 0.8 - 10) / (LW * FM) - 1;
                coord.fgcolor = selcolor;
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    tft.drawFastHLine(12, 39, tftWidth - 24, bruceConfig.secColor);
    tft.drawString(title, 12, 30);
    drawModuleDipStatus(title);
...
void drawStatusBar() {
    ...
    if (bruceConfig.theme.border) {
        ...
        tft.drawLine(8, 27, tftWidth - 9, 27, bruceConfig.secColor);
        tft.drawPixel(8, 8, bruceConfig.secColor);
        tft.drawPixel(tftWidth - 9, 8, bruceConfig.secColor);
        tft.drawPixel(8, tftHeight - 9, bruceConfig.secColor);
        tft.drawPixel(tftWidth - 9, tftHeight - 9, bruceConfig.secColor);
    }
...
void printSubtitle(String subtitle, bool withLine) {
    ...
    if (withLine) {
        tft.drawFastHLine(cursorX, tft.getCursorY(), subtitle.length() * FP * LW, bruceConfig.secColor);
    }
}
